...
export default function LandingIntro() {
  ...
  /* Check WebGL once on mount (mobile can be flaky) */
  useEffect(() => {
    try {
  ...
  const flow = [
    { text: "Hey there — welcome to HealHub Center.", next: 1 },
    { text: "I’m Bimpe, your personal wellness companion.", next: 2 },
    {
      text: "Before we start, how are you feeling today?",
  ...
  const responses = {
    "I'm good": {
      text: "Love that energy. Want to explore the community or start a quick chat?",
      options: ["Join community", "Chat with Bimpe"],
    },
    "I'm okay": {
      text: "No rush. Want a 30-second tour to get settled?",
      options: ["Take a quick tour", "Maybe later"],
    },
    "Not great": {
      text: "You’re safe here. Would you like to talk to me or join a roundtable?",
      options: ["Chat with Bimpe", "Join roundtable"],
    },
  };

  /* Step auto-advance for the intro lines */
  useEffect(() => {
    if (step < 2) {
      const id = setTimeout(() => setStep((s) => s + 1), 3000);
      return () => clearTimeout(id);
    }
  }, [step]);

  /* CTA handling — all guarded routes go to /login (per your request) */
  ...
  return (
    ...
    >
      ...
      >
        {/* ✅ Canvas with mobile-friendly settings + graceful fallback */}
        {webglOK ? (
          <Canvas
            // Important: keep animations alive on mobile
            frameloop="always"
            // Lower DPR on phones; helps prevent context loss/black screens
            dpr={[1, isMobileUA ? 1.25 : 2]}
            // Keep it light on mobile
            shadows={!isMobileUA}
            gl={{
              antialias: !isMobileUA, // disable AA on phones for stability
              alpha: true,
          ...
          >
            ...
            <directionalLight
              name="breatheLight"
              position={[4, 5, 3]}
              intensity={isMobileUA ? 0.8 : 1}
              color="#FFEBD2"
              castShadow={!isMobileUA}
            />

            {/* Env map is heavier; skip it on mobile */}
            {!isMobileUA && <Environment preset="city" />}
          ...
          // Static fallback (if WebGL is blocked/unsupported)
          <div
            style={{
              position: "absolute",
              inset: 0,
              background:
                "radial-gradient(1200px 800px at 50% 65%, #F7F3EE 0%, #FBF9F5 60%, #FFFFFF 100%)",
            }}
          />
        )}
      </motion.div>
        ...
        {!finalMsg && currentDialogue?.text && (
          <motion.div
            ...
            style={{
              position: "absolute",
              bottom: "12%",
              left: isMobileUA ? "8%" : "16%",
              transform: "translate(-10%,0)",
              background:
                "linear-gradient(135deg,rgba(255,255,255,0.95),rgba(245,240,250,0.92))",
              borderRadius: "14px 14px 14px 6px",
              padding: isMobileUA ? "0.8rem 0.9rem" : "0.9rem 1.1rem",
              maxWidth: isMobileUA ? 280 : 320,
              fontSize: isMobileUA ? "0.9rem" : "0.95rem",
              color: "#111",
          ...
          <motion.div
            ...
            style={{
              ...
              background:
                "linear-gradient(135deg,rgba(255,255,255,0.95),rgba(245,240,250,0.92))",
              borderRadius: 12,
              padding: isMobileUA ? "0.7rem 0.9rem" : "0.7rem 1rem",
              maxWidth: isMobileUA ? 280 : 320,
              fontSize: "0.92rem",
              color: "#111",
              boxShadow: "0 10px 24px rgba(0,0,0,0.10)",
